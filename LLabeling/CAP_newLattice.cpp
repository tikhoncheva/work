...
MatrixDouble triangularLattice2(int m1, int m2) {
	MatrixDouble coord;
	int k=0;
...
MatrixDouble squareLattice2(int m1, int m2) {
	MatrixDouble coord;
	int k = 0;
	coord.resize(m1*m2);
...
int CProblem::setProblem() {
	...
	switch (lattice) {
		...
		case 2:
			...
			m2 = 4;
			n=m1*m2;
			coordinates = triangularLattice2(m1, m2); // number of vertices, number of vertices in first line
			diameter = 6;
		...
		case 3:
			...
			m2 = 5;
			n=m1*m2;
			coordinates = squareLattice2(m1,m2);
			diameter = 8;

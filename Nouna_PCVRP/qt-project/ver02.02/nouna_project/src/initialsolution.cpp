...
std::vector<std::vector<std::pair<unsigned int, double> > > planInterviews_dayly(
                                                        ...
                                                        std::vector<std::vector<unsigned int> > _village_households)
{
    /*
...
//    std::vector<std::vector<unsigned int> > dayVillages(nDays);
//    std::vector<stHousehold>::iterator hh_it;
//    unsigned int vID;

    // maximal worktime pro day (in min)
    const double tmax = K * 8*60.;
    ...
    for (unsigned int i=0; i<nLongInterviews; ++i)
    {
        ...
        itime = planLongITimes[0][i].second;
/*
        hh_it = std::find_if(_households_sorted.begin(), _households_sorted.end(),
                               [hhID] (stHousehold const& hh)
                                 { return hh.ID == (hhID+10001);});
        vID = (*hh_it).villageID-101;
*/
...
/*
        if (std::find(dayVillages[w*5+d].begin(), dayVillages[w*5+d].end(), vID) == dayVillages[w*5+d].end());
        {
            dayVillages[w*5+d].push_back(vID);
            dayVillages[constant::nweeks*5 + w*5 + d].push_back(vID);
            dayVillages[2*constant::nweeks*5 + w*5 + d].push_back(vID);
        }
*/
...
//    for (d=0; d<nDays; ++d)
//    {
//        for(unsigned int v=0; v< dayVillages[d].size(); ++v)
//        {
//            vID = dayVillages[d][v];
//            for (unsigned int h=0; h<_village_households[vID].size(); ++h)
//            {
                     ...
                     )
{
     ...
     * Preprocessing
     */
    households_sorted = households_presorting(_villages,_households,_village_households);

    /*
     * Associate interviews with days
     */
    _hhITimePlan_dayly =  planInterviews_dayly(households_sorted, nI, _TimeInfo, _village_households);

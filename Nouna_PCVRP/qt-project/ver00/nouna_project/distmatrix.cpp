...
std::vector<std::vector<double> > compute_distmatrix(std::vector<stVillage> Village,
...
//    std::vector<std::vector<stRoute_short> > distmatrix;
    ...
    for (unsigned int i=0; i<nV; i++)
        distmatrix[i] = std::vector<double>(nV,0.);
...
*/
        distmatrix[i][j] = Route[r].dist;//tmpRouteInfo;
    }

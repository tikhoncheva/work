 ...
 *
 */
std::vector<std::vector<unsigned int> > planInterviewInYear(std::vector<stHousehold> _households_sorted, // _households
...
//        ToVis[hhID] = false;
    ...
    /*
     * Step 3: assign a day to each interviewer according to the week assignments from the
     * last step
     */
    ...
    std::vector<std::vector<unsigned int> > dayInterviews(nDays);
...
//                                - 5* constant::maxDistRain *     rainingSeason;
    ...
    for (unsigned int i=0; i<nLongInterviews; ++i)
    {
        ...
        dayInterviews [w*5+d].push_back(hhID);
        remaining_time[w*5+d] -= itime;

        dayInterviews [constant::nweeks*5 + w*5 + d].push_back(hhID);
        remaining_time[constant::nweeks*5 + w*5 + d] -= planLongITimes[1][i].second;

        dayInterviews [2*constant::nweeks*5 + w*5 + d].push_back(hhID);
    ...
    {
        hhID  = _households_sorted[indShortInterviews[i]].ID;
        itime = _households_sorted[indShortInterviews[i]].itime;

        dayInterviews [d].push_back(hhID);
        remaining_time[d] -= itime;

        dayInterviews [constant::nweeks*5 + d].push_back(hhID);
        remaining_time[constant::nweeks*5 + d] -= itime;

        dayInterviews [2*constant::nweeks*5 + d].push_back(hhID);
                     ...
                     )
{
    ...
    unsigned int nV = _households.size();
     ...
     */
    std::vector <std::vector <unsigned int> > dayplan_interviews;

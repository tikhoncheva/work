	...
	try {
		...
		switch (lattice){
		  case 1: // hexagonal_lattice 
		    if (max_d==3) M = 10;	//d(x) = 3 -x
		    else M = 27;
		    break;
		  case 2:// triangular_lattice
		    if (max_d==3) M = 16;	//d(x) = 3 -x
		    else M = 54;
		    break;
		  case 3:// square_lattice
		    if (max_d==3) M = 12;	//d(x) = 3 -x
		    else M = 33;
		    break;
		  default: M=n*max_d;break;
		}
		std::cout << "Lattice " << lattice << "\n";
		std::cout << "UB for lambda " << M << "\n";
		UB = M;
		...
		/* solve the Model*/
		cplex.extract(model);
		cplex.exportModel("LLabeling.sav");

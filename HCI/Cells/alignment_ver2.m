...
aligned_images = uint8(zeros(3*m, 3*n, nImages));

img = images_fs(:,:,1);
aligned_images(m+1:2*m,n+1:2*n,1) = img;
...
for k = 2:nImages
    ...
    for i = 1:numel(yshift)
        ...
        for j = 1:numel(xshift)
            ...
            c_max = max(abs(c(:)));
            [ypeak, xpeak] = find(abs(c)==c_max);           % position of path in img
    ...
    dx = x0 - x;
    dy = y0 - y;
    aligned_images(m+1+dy:2*m+dy, n+1+dx:2*n+dx,k) = img2; 
...
%     img1 = img2;
    img = aligned_images(m+1+dy:2*m+dy, n+1+dx:2*n+dx,k);
end

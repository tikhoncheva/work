function [val, I, J] = patch_similarity(hog1, hog2, convsize)
% HOGSIM - Calculates the similarity between whiten HOG descriptors
%
...
parfor k = 1:numel(hog1)*numel(hog2)
     ...
     H2p = zeros(size(H1) + [2*convsize,2*convsize,0]);

     M = min(size(H2,1), size(H2p,1));
     N = min(size(H2,2), size(H2p,2));

     H2p(floor((size(H2p,1)-M)/2)+[1:M],
floor((size(H2p,2)-N)/2)+[1:N],:) = ...

...
%    videoName = videoName(8:end-8);
   ...
   vel_wheel_l1 = zeros(1,4);      % vector of estimated wheel velocity between two consecutive frames
%    vel_wheel_l2 = zeros(1,4);      % vector of estimated wheel velocity between two consecutive frames
   vel_wheel_l1_greedy = zeros(1,4);      % vector of estimated wheel velocity between two consecutive frames
   
   %%
   for i=2:nFrames
        fprintf('iteration %d from %d', i, nFrames);
        ...
        vel_wheel_l1(1,:) = [ind_prev, ind_next, theta, er];
        
        [theta1, er1] = greedy_ME( sel_points, V, wheel_param, 'l1');
        vel_wheel_l1_greedy(1,:) = [ind_prev, ind_next, theta1, er1];
...
%    fclose(fileID);
   ...
   % save estimaed wheel motion (rad/frame) in a file
   fileID = fopen([path_d4, videoName, '_wheel_motion_l1_greedy.txt'],'w');

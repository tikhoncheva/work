function [ circles ] = circle_detection2( img, votes_dir, cyRange, cxRange, rRange, nclusters, varargin )
tic
...
H = HoughTr_circles2(img, cyRange, cxRange, rRange);
...
% select most prominent picks for each radius
npicks_per_r = 10;
...
%    H_r = imfilter(H_r, h);
   ...
   cy = picks_pos( ind(1:min(npicks_per_r, size(picks_pos,1)) ), 1);
   cx = picks_pos( ind(1:min(npicks_per_r, size(picks_pos,1)) ), 2);
...
nclusters = min(nclusters, size(circles,1));
...
% [ind, centers] = kmeans(circles(:,1:2), nclusters, 'EmptyAction', 'drop');
% 
% circle_cluster = [];
% for i=1:nclusters
%     rad = circles(ind==i, 3);

...
% Modification of the initial code apclusterSoarse.m by Frey and Dueck
...
% function called net similarity.
...
%
% N     data points
% s     (N^2-N)x3 list of pair-wise similarity between the nodes:
%       s(i,3) is the similarity of data point s(i,1) to data point s(i,2)
%       (i.e. list of the entries of the NxN similarity matrix sim_mat with 
%       zeros on the diagonal. s(i,k) is the similarity of point i to point k)
% p     Nx1 matrix of preferences: p(i) indicates the preference that data
%       point i be chosen as a cluster center
%       TIPP (Frey and Dueck) : A good choice is to set all preference
%       values to the median of the similarity values. The number of 
%       identified clusters can be increased or decreased  by changing 
%       this value accordingly. If p is a scalar, APCLUSTER assumes 
%       all preferences are equal to p
%
% PARAMETERS: The following parameters can be set by providing name-value
...
% OUTPUT
% idx           cluster centers: idx(j) is the index of the data point 
%               that is the cluster center for data point j
% netsim        sum of the data point similarities and preferences (net similarity)
% dpsim         the sum of the similarities of the data points to their
%               cluster centers
% expref        sum of the preferences of the identified cluster centers
%
%
%%
function [idx,netsim,dpsim,expref] = apclusterSparse_sumprod(s,p,varargin)
...
tmp1_1 = tril(A,-1); tmp1_1 = [tmp1_1(2:end,:), zeros(N-1,1)];
tmp1_2 = triu(A);    tmp1_2 = [zeros(N-1,1), tmp1_2(1:end-1,:)];

tmp1 = [tmp1_1+tmp1_2;b];

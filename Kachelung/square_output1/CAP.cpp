...
int CProblem::setModel()
{
	...
	try
	{
		...
		if (max_distance == 2) 
		{
		  ...
		  // Vierecke mit Seitenlaenge 2 mit Ueberlappungen
		  for (int i = 0; i < sqrt(n)-2; i+=1)
		  {
		    for (int j = 0; j < sqrt(n)-2; j+=1)
		    {
			  model.add(c[i * sqrt(n) + j] + c[i * sqrt(n) + j+2] +
			  c[(i+2) * sqrt(n) + j] + c[(i+2) * sqrt(n) + j+2] >= 2.68629);
		    }
		  }
		  
		  // Vierecke mit Seitenlaenge 3 mit Ueberlappungen
		  for (int i = 0; i < sqrt(n)-3; i+=1)
		  {
		    for (int j = 0; j < sqrt(n)-3; j+=1)
		    {
			  model.add(c[i * sqrt(n) + j] + c[i * sqrt(n) + j+3] +
			  c[(i+3) * sqrt(n) + j] + c[(i+3) * sqrt(n) + j+3] >= 0);
		    }
		  }
		...
		if (max_distance == 3) 
		{
		  ...
		  // Vierecke mit Seitenlaenge 2 ohne Ueberlappungen
		  for (int i = 0; i < sqrt(n)-2; i+=1)
		  {
		    for (int j = 0; j < sqrt(n)-2; j+=1)
		    {
			  model.add(c[i * sqrt(n) + j] + c[i * sqrt(n) + j+2] +
			  c[(i+2) * sqrt(n) + j] + c[(i+2) * sqrt(n) + j+2] >= 8.68629);
		    }
		  }
		  
		  // Vierecke mit Seitenlaenge 3 ohne Ueberlappungen
		  for (int i = 0; i < sqrt(n)-3; i+=1)
		  {
		    for (int j = 0; j < sqrt(n)-3; j+=1)
		    {
			  model.add(c[i * sqrt(n) + j] + c[i * sqrt(n) + j+3] +
			  c[(i+3) * sqrt(n) + j] + c[(i+3) * sqrt(n) + j+3] >= 2);
		    }
		  }

...
int CProblem::setModel()
{
	...
	try
	{
		...
		if (max_distance == 2) 
		{
		  ...
		  // Quadratische Gridgraphen mit 4 Ecken ohne Ueberlappungen
		  for (int i = 0; i < sqrt(n)-2; i+=2)
		  {
		    for (int j = 0; j < sqrt(n)-2; j+=2)
		    {
			  model.add(
			    c[i * sqrt(n) + j] + c[i * sqrt(n) + j+1] +  c[i * sqrt(n) + j+2] +
			    c[(i+1) * sqrt(n) + j] + c[(i+1) * sqrt(n) + j+1] +  c[(i+1) * sqrt(n) + j+2] +
			    c[(i+2) * sqrt(n) + j] + c[(i+2) * sqrt(n) + j+1] + c[(i+2) * sqrt(n) + j+2] >= 29.1922);
		    }
		  }
		  ...
		  for (int i = 0; i < sqrt(n)-2; i+=2)
		  {
		    for (int j = 0; j < sqrt(n)-2; j+=2)
		    {
			  model.add(
			    c[i * sqrt(n) + j] + c[i * sqrt(n) + j+1] +  c[i * sqrt(n) + j+2] +
			    c[(i+1) * sqrt(n) + j] + c[(i+1) * sqrt(n) + j+1] +  c[(i+1) * sqrt(n) + j+2] +
			    c[(i+2) * sqrt(n) + j] + c[(i+2) * sqrt(n) + j+1] + c[(i+2) * sqrt(n) + j+2] >= 59.4033);
		    }
		  }

 ...
 * return one matrix instead of two arrays
 */
...

{
    ...
    double pmeanLongITime = constant::summaryLongITime / double(constant::P);
    double wmeanLongITime = constant::summaryLongITime / double(constant::P * constant::nweeks);
    ...
    nLongInterviews = constant::nLongInterviews;                        // correct number of long interviews
    ...
    assert(indShortInterviews.size() == constant::nShortInterviews);    // correct number of short interviews
    ...
    */
    ...
    w = 0;
    for (unsigned int i=0; i<constant::nShortInterviews; ++i)
    {
...
//                                   const std::vector<std::vector<double> >  _timematrixDry,// distmatrix
//                                   const std::vector<std::vector<double> >  _timematrixRain)
{
    ...
    for (unsigned int w = 0; w<nWeeks; ++w)
    {
        ...
        tmax = 5*8*60. - 5 * constant::maxTimeDistDry  *(1 - rainingSeason)
                       - 5 * constant::maxTimeDistRain *     rainingSeason;
                                    ...
                                    const std::vector<std::vector<double> >  _timematrixRain)
{
    ...
    for (unsigned int w = 0; w<nWeeks; ++w)
    {
        ...
        for (unsigned int i=0; i<hhToVisit.size(); ++i)
        {
            ...
            thome =    rainingSeason *_timematrixRain[nextV][home] // if interviewer would come back home after this hh
                    + (1-rainingSeason)*_timematrixDry[nextV][home];
            tmove =    rainingSeason *_timematrixRain[predV][nextV] // if interviewer would come back home after this hh
                    + (1-rainingSeason)*_timematrixDry[predV][nextV];
...
void make_day_plans (stInterviewer& interviewer,
                     ...
                     const std::vector<std::vector<double> > _timematrixDry,        // distmatrix
                     const std::vector<std::vector<double> > _timematrixRain        // distmatrix
                     )
{
    unsigned int nWeeks = constant::nweeks * constant::P;
    ...
    for (unsigned int w=0; w<nWeeks; ++w)
    {
        ...
        //        vToVis = sort_villages_dist(vToVis,_timematrixDry);
        ...
        for (unsigned int v=0; v<V; ++v)    // for each village
        {
            ...
            while (!village_household_Matrix[v].empty())
            {
                ...
                for (d=0; d<=day; ++d)
                {
                    ...
                    tmove =    rainingSeason *_timematrixRain[predV][nextV] // time to change village
                            + (1-rainingSeason)*_timematrixDry[predV][nextV];
                ...
                if (d>day)
                {
                    ...
                    tmove =    rainingSeason *_timematrixRain[predV][nextV] // travel time between previous village and new village
                            + (1-rainingSeason)*_timematrixDry[predV][nextV];
        ...
        {
            predV = visitedV[d].back();
            thome =    rainingSeason *_timematrixRain[predV][home]
                    + (1-rainingSeason)*_timematrixDry [predV][home];
...
void stay_over_night(std::vector<stInterviewer>& _interviewer,
                     std::vector<std::vector<double> >  _timematrixDry,  // distmatrix
                     std::vector<std::vector<double> >  _timematrixRain) // distmatrix)
{
    unsigned int nWeeks = constant::nweeks * constant::P;
    unsigned int nDays = 5;
            ...
            for (unsigned int day=0; day<nDays-1; ++day)
            {
                ...
                if ( len >= 2 && len1 >= 2)
                {
                    ...
                    if (lastV == firstV)
                    {
                        ...
                        thome =    rainingSeason *_timematrixRain[lastV][home] // travel time between previous village and new village
                                + (1-rainingSeason)*_timematrixDry [lastV][home];
                        ...
                        thome =    rainingSeason *_timematrixRain[home][firstV] // travel time between previous village and new village
                                + (1-rainingSeason)*_timematrixDry [home][firstV];
                        ...
                        std::vector<std::vector <std::pair<unsigned int, double> > >& _hhITimePlan_week)
{
    ...
    do {
        ...
        tmove =    rainingSeason *_timematrixRain[predV][V]
                + (1-rainingSeason)*_timematrixDry[predV][V];

        thome =    rainingSeason *_timematrixRain[V][home]
                + (1-rainingSeason)*_timematrixDry[V][home];
...
/*
            ...
            thome_pred =    rainingSeason *_timematrixRain[newVVec.back()][home]
                    + (1-rainingSeason)*_timematrixDry [newVVec.back()][home];
            twork += thome_pred;
            newVVec.push_back(home);
            ...
            if (overhours)
            {
                ...
                if (freetime_right > freetime_left)
                    propagateToTheRight(w /*week*/, day_l /*day*/,
                                        ...
                                        _timematrixDry /* distmatrix */,
                                        _timematrixRain /* distmatrix */,
                      ...
                      )
{
    ...
    _hhITimePlan_weekly = planInterviews_weekly(_households_sorted, nI);
    std::cout << " ... finished\n" << std::endl;


    std::cout << "Make week plans for each interviewer";
    assignInterviewersToHH_weekly2(_hhITimePlan_weekly, _households,_interviewer,
                                   _timematrixDry, _timematrixRain);
    std::cout << " ... finished\n" << std::endl;


    std::cout << "For each interviewer assign days of the visitis";
    for (unsigned int k=0; k<_interviewer.size(); ++k)
        make_day_plans(_interviewer[k], _villages, _households,_hhITimePlan_weekly, _timematrixDry, _timematrixRain);
    std::cout << " ... finished\n" << std::endl;


    std::cout << "Split too long interviews into several days";
    split_longinterviews(_interviewer, _households, _timematrixDry, _timematrixRain, _hhITimePlan_weekly);
    std::cout << " ...finished" << std::endl;


    std::cout << "Allow to stay over the night in some villages";
    stay_over_night(_interviewer, _timematrixDry, _timematrixRain);
    std::cout << " ...finished\n" << std::endl;

 ...
 * -------------------------------------------------------------------------------------------------
 */
void assignInterviewersToHH_weekly(std::vector<stInterviewer>& _interviewer,
                                   ...
                                   const std::vector<std::vector<double> >  _distmatrixRain)
{
    ...
    std::vector<std::vector<unsigned int> > groups_V; // visited villages
    std::vector<std::vector<unsigned int> > groups_Hh;// visited households
    std::vector<unsigned int> groups_remaining_time;// visited households

    std::vector<unsigned int> group_V; // visited villages
    std::vector<unsigned int> group_Hh;// visited households

    stRoute tmpRoute;

    // maximal worktime pro week (in min)
    double tmax = 5*8*60.;
    double ti;                     // interview time
    double thome;                   // time to return to Nouna
    ...
    for (unsigned int w = 0; w<nWeeks; ++w)
    {
        ...
        for (unsigned int i=0; i<hhToVisit.size(); ++i)
        {
            ...
            if (remaining_time - (tmove + ti + thome) < 0)
            {
                groups_Hh.push_back(group_Hh);
                groups_V.push_back(group_V);
                groups_remaining_time.push_back(remaining_time);

                group_V.erase(group_V.begin(), group_V.end());
                group_Hh.erase(group_Hh.begin(), group_Hh.end());
                remaining_time = tmax;
            }

            remaining_time -= (tmove + ti + thome);
            group_Hh.push_back(hhID);
            group_V.push_back(nextV);

        }   // end build groups

        // wenn all groups are built
        // assign them to interviewer


        if (groups_Hh.size()>nI)
            std::cout << "Error in week plans: " << groups_Hh.size() << " groups vs "
                      << nI << " interviewers" << std::endl;

        unsigned int iID = 0;  // ID of an interviewer
        for (unsigned int g=0; g<groups_Hh.size(); ++g)
        {

            tmpRoute.villages = groups_V[g];
            tmpRoute.households = groups_Hh[g];
            tmpRoute.time = tmax - groups_remaining_time[g];

            _interviewer[iID].routes_weeks.push_back(tmpRoute);
//            iID = std::max(iID + 1, nI-1);//////////////////////////////////////////////////

            groups_V[g].erase(groups_V[g].begin(), groups_V[g].end());
            groups_Hh[g].erase(groups_Hh[g].begin(), groups_Hh[g].end());
        }

        groups_V.erase(groups_V.begin(), groups_V.end());
        groups_Hh.erase(groups_Hh.begin(), groups_Hh.end());
        groups_remaining_time.erase(groups_remaining_time.begin(), groups_remaining_time.end());
                      ...
                      )
{
    ...
    assignInterviewersToHH_weekly(_interviewer, _households,_hhITimePlan_weekly,

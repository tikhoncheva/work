...
public:
	...
	void print();
	int setModel();
	int save_result(std::string _fileName);
};
...
int CProblem::setModel() {
	...
	IloEnv   env;
	   try {
	      ...
	      /* d=(d_1, d_2, ..., d_s) */
	      std::cout << " Set Model d: ";
 	      IloIntArray Par_d(env, d.size());
	      for (unsigned int i=0; i<d.size(); i++) {
	      	  Par_d[i]=d[i];
		  std::cout << Par_d[i] << " "; 
	      }
	      ...
	      std::cout << " Set Model constraints: ";
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!		  
	    ...
	    /* solve the Model*/
	      ...
	      double const start = cplex.getCplexTime();
	      
	      int solveError = cplex.solve();
	      
	      double const elapsed =  cplex.getCplexTime() - start;
	      ...
	      env.out() << "Solution time with cplex.getCplexTime()  = " << elapsed << "\n";
...
int CProblem::save_result(std::string fileName){
